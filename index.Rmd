...
All the time I was building and analyzing models, I was looking for a way of organizing, operationalizing, and applying the rich body of theory developed over more than a century of malaria research. 
In 1800, malaria was a disease characterized by recurring fevers. 
By the 19th century, malaria was treated with a tea made from the bark of the Cinchona tree and then quinine, which was isolated in 1824. Later in the 19th century, observations made by Charles Laveran and Ronald Ross through light microscopy identified the cause of malaria and its mode of transmission 
[@RossR1911Book;@SmithDL2012_RossMacdonald]. In the 1900s, Ross developed the first mathematical models of malaria transmission. 
In the early 1950s, George Macdonald's quantitative synthesis of malaria epidemiology updated the mathematical theory.
Klaus Dietz addressed some of the major failings of the Ross-Macdonald model in the 1970s, including publication of a new malaria model that was field tested in a malaria control trial called The Garki Project. 
In the 1980s, mathematical models of malaria and other infectious diseases were taken up by departments of ecology, leading to innovation [@ReinerRC2013SystematicReview]. 
In the 1990s, within-host models were developed [@MolineauxL1999ReviewIntrahost].
Comprehensive individual-based malaria simulation models were developed at the Swiss Tropical Public Health Institute (2006), Imperial College (2010), and the Institute for Disease Modeling (2013). 
I had been meanwhile working towards a mathematical and computational framework that was *not* an individual-based simulation model that could integrate the concepts and models that have influenced malaria through the present day [@ReinerRC2013SystematicReview;@SmithNR2018AgentbasedModels].
I was also looking for a way of recasting theory for malaria epidemiology, transmission dynamics, and control to address some outstanding, unanswered questions [@SmithDL2014Recasting].
The big vision was a tool for translating evidence into policy that was powerful enough to handle the complexity of malaria and the uncertainty about malaria epidemiology but that could be used to identify relevant details about malaria transmission dynamics and control for malaria policy.


In collaboration with Uganda's National Malaria Control Division (NMCD) and the Bioko Island Malaria Elimination Program (BIMEP), we developed a malaria policy framework that we call ***adaptive malaria control,*** supported by a bespoke inferential framework that we have called ***robust analytics for malaria policy (RAMP):***
analytics are robust if they characterize, quantify, and propagate uncertainty. 
Since malaria is so complex, a core part of adaptive malaria control is using mathematical models for simulation-based analytics through iterative policy engagement. 
...
Comprehensive individual-based simulation models had been developed to support malaria policy, but there were serious practical barriers to using these models outside of academic settings.
I was looking for a more practical way of handling the complexity. 
...
Sometime in the fall of 2022, the last few pieces came together. We published the first versions of [MicroMoB](https://cran.r-project.org/package=MicroMoB) and [exDE](https://CRAN.R-project.org/package=exDE) at CRAN. 
