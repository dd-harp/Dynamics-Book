...
output:
  ...
  bookdown::pdf_book:
    ...
    extra_dependencies:
      ...
      caption: null
documentclass: book
bibliography: ../RAMP-Model-Library/references.bib
csl: ../RAMP-Model-Library/plos.csl
link-citations: true
...
A large fraction of my time over the past 20 years has been devoted to learning about malaria epidemiology, mosquito ecology, and vector control. All the time I was building and analyzing models, I was looking for a way of organizing and applying the rich body of theory that has been developed over more than a century of malaria research starting with the Ross-Macdonald model [@SmithDL2012_RossMacdonald] and building on more recent research [@ReinerRCJ2013SystematicReview;@SmithNR2018AgentbasedModels]. A new framework would require rethinking the way we implement models. Ideally, it would be *extensible* with *plug-and-play* modularity and structural flexibility. It should include models that could scale down for fine-grained simulations, or to scale up to understand regional spatial patterns. To serve the needs of malaria programs, a framework would need to be capable of modeling malaria as a changing baseline modified by control. The framework would need built-in support to model exogenous forcing by weather and vector control. To get integrated malaria control right, we needed to go all in on mosquito ecology. This required developing generic interfaces for the major components, including some new models and algorithms. New models, algorithms, or theory were developed for blood feeding, heterogeneous exposure, human mobility, cohort dynamics, and mosquito dispersal. We wanted the software to help compute thresholds for malaria transmission in heterogeneous systems. Sometime in the fall of 2022, the last few pieces came together. We published the first versions of [MicroMoB](https://cran.r-project.org/package=MicroMoB) 
and [exDE](https://CRAN.R-project.org/package=exDE) at CRAN, submitted a paper to PLoS Computational Biology [@WuSL2022SpatialDynamics], and realized it was time to write this book. 

This book is about building malaria models that are up to the task of guiding malaria policy. It fills a gap that comes after taking an introduction to mathematical epidemiology or infectious disease modeling. What are all the special topics that would need to be covered to build models that could be needed in malaria? This book *could* be the basis for a course, if there were ever enough students. Since there will probably never be enough graduate students who are interested in applied malaria dynamics to support a course at a university, we have developed the material for all the students who might needed a course like this anywhere on the planet. 

The premise of this book is thus that the reader has a solid background in infectious disease models and malaria. We assume they've seen the Ross-Macdonald model before, and that they know something about how to construct models. Our goal is to teach the material through examples, so we won't try to fill in any technical details in the text. Instead, we have written some vignettes and lessons to accompany the book in the `RAMP-Model-Library`.  

This book teaches about model building for malaria policy. We present some motivating examples, but we stop short of fitting models to data or applying models to policy. Some of those topics are being taken up in a companion [**Robust Analytics for Malaria Policy.**](../../RAMP-Book/_book/index.html){target="_blank"}
