...
+ Section \@ref(QLogic) present's Ross's *quantitative logic.* 

+ Section \@ref(dtsRoss) presents a basic deterministic discrete-time system. Since it is the first model in the book, we go slowly, walking through the model step-by-step and discussing the basics in detail.  

+ In Section \@ref(basicComputation), we discuss some basic issues about coding and computation.  

+ In Section \@ref(countingEvents), we show that the model presented in Section \@ref(dtsRoss) is numerically unstable. To *fix* the numerical instability, we introduce the Poisson model, a probability distribution function describing a counting process. We use the idea to present a new deterministic discrete-time system of equations that is numerically stable. 

+ Section \@ref(sdtsRoss) presents a stochastic discrete-time system, where the state of the system is updated each time step using random variables. 

+ Section \@ref(xdeRoss) presents a deterministic continuous-time system. 

+ Section \@ref(sxdeRoss) presents a stochastic continuous-time system with *asynchronous* updating: events in the system occur one at a time, and the system is updated after each event

+ In Section \@ref(mathSpecies), we compare and contrast discrete *vs.* continuous, and deterministic *vs.* stochastic models. 

+ In Section \@ref(basicChNotes), we discuss some of the discrete-time systems that played a role in development of theory for infectious diseases. 

The goals for this chapter are to introduce the basic elements of models, to present several different different kinds of models, to develop some basic intuition about how they work and their differences, and to emphasize that all models as approximations.
In this chapter, we're trying to develop basic skills that we can use later. There's a lot of basic material to digest, and since we'll use it later, we're not trying to delve too deeply into malaria epidemiology. Each model is implemented as R code^[In developing this book, we assumed the reader would be familiar with R and RStudio.]
...
## Ross's Quantiative Logic {#QLogic}
...
To explain these differences, he developed mathematical models that described 
the processes affecting how malaria prevalence would change over time:
infection and the loss of infection. 
...
Malaria infections in two host populations -- humans and mosquitoes -- are connected through mosquito blood feeding, so malaria transmission works as an integrated system with two otherwise independent compoonents. The process looks something like the following diagram.
...
This graph is the basis for causal theory for understanding malaria parasites in populations. 
A student of causation might object that the graph describes a cycle and thus it can not be causal. 
...
We're trying to develop a toolbox that we can use to investigate malaria transmission dynamics and control. 
...
The discrete time model `ross_diffs_1` can be implemented in a spreadsheet. We did one as a [Google Sheet.](https://docs.google.com/spreadsheets/d/1gM6VoHJSQcEUi96rOjLaGKLO5tnuAgENIz50Q6TAjZY/edit?gid=0#gid=0)
...
Also of interest, discrete time model `ross_diffs_2` was implemented in a [Google Sheet.](https://docs.google.com/spreadsheets/d/1gM6VoHJSQcEUi96rOjLaGKLO5tnuAgENIz50Q6TAjZY/edit?gid=0#gid=0) 
...
If we followed individuals in a population that followed the simple rules described in Equation \@ref(eq:Mprocess) and Equation \@ref(eq:Hprocess), we would like to be able to check their state every day.
Each individual would be either *infected* or *uninfected.* 
The number of infected individuals would have to be a counting number. 
The equations from the last chapter would return real numbers, but no one can be half-infected. 
We will also need to be able to deal with random changes in an individual's status from day to day. 
We need a new kind of model to deal with this.

understand how they are connected to the deterministic equations we developed in  Section-\@ref(dtsRoss). Setting aside the problems that get introduced by sampling populations for the moment -- we will pretend we're omniscient -- we would find random changes occurring:
