...
We start out presenting Ross's *quantitative logic,* and then we present several different mathematical models that implement the same basic logic. Ross gets credit for two of these.  

1. Section \@ref(dtsRoss) presents a basic deterministic discrete-time system. Since it is the first model in the book, we walk through it step-by-step, discussing the basics in detail and going slowly.

2. In Section \@ref(basicComputation), we discuss some basic issues about coding and computation.  

3. In Section \@ref(countingEvents), we show that the model presented in Section \@ref(dtsRoss) is numerically unstable. To *fix* the numerical instability, we introduce the Poisson model, a probability distribution function describing a counting process. We present a new deterministic discrete-time system of equations that is numerically stable. 

4. Section \@ref(sdtsRoss) presents a stochastic discrete-time system; events are updated each time step 

5. Section \@ref(xdeRoss) presents a deterministic continuous-time system; 

6. Section \@ref(sxdeRoss) presents a stochastic continuous-time system; events are updated asynchronously 
