...
To develop a mathematical model, Ross believed that it was possible to describe the process *a priori* -- or in other words, to formulate mathematical models using quantitative logic. 
To write down the model, we must make assumptions.
...
Since infections don't xy forever, the prevalence of infection in mosquitoes and humans reflects a balance between infections acquired through parasite transmission and the natural loss of infection through parasite clearance or host mortality.
...
In this section, we present the basic, deterministic, discrete-time model for a mosquito-transmitted pathogen that does not immunize its host against subsequent infection. Because it is the first model in the book, we'll take the time to explain some background material in detail. We'll discuss the parts of a model, and we walk through sections about solving, computation, analysis, and model verification. The first model we present, called `ross_dts_1,` is numerically unstable in some cases. At the end we present another model, called `ross_dts_2` that is numerically stable. 

### `ross_dts_1`

We will present the parts of the model first, and write down the equations xy. The parts of a model are called parameters and variables. To solve the model, we will need initial values for the variables. As we go, we will also write R code that implements the model and that solves it. In writing the code, we will adopt some conventions that we can use to help us write code that is easy to read and use. 

**Variables** are quantities that describe the states of a system that we want to understand and compute.
...
We can now begin to write R code to set up the objects so we can compute them. We set these initial values to be close to zero: 
...
In R, this object `xy` is called a *named vector.* The names are carried along but they don't affect any computation done with `xy.` It is also useful that the names often get inherited (but not always). The names can be used with `as.list()` or `data.frame().` When combined with the `with()` function, we can create a context where we can get the values by name: 
...
We could rewrite the equations to so that they correspond directly to statements like *The change in...* is equal to *a process description,* where the first statement is the left hand side, and the second one is the right hand side:
...
In this alternative way of writing down the equations, the left hand side is interpreted as *the daily change,* and the terms on the right hand side describes those changes. The RHS has four terms. The first two describe the change in $x$:
...
The next two describe the change in $y$:
...
To foreshadow something we want to address later (in sub-section \@ref(dtsRoss2)), if $may_t >1,$ then it is not a proportion. In that case, the equations don't make sense and they will return nonsense if we use them.

***

To update the variables, we write a function in R. Since we will be developing a lot of functions and models, we adopt a simple naming convention: since a version of it was first developed by Ross, we attach the stem `ross`; since this is a discrete time system, we attach the suffix `dts`; we might want to generate other variants, so we append a number. The function is thus called `ross_dts_1.` 
...
We can also take a peak at the object `xy_t:` 
...
We can look at the xy few values, the values of $t,$ $x$ and $y$ are stored in columns: 
...
Computers are very good at repeating tasks, but if we want to get the computers to repeat tasks efficiently, we must write the code to do it. One useful trick is to write functions that call other functions to repeat common workflows. In this case, we want to write a function that *solves* the equations (*i.e.* that iteratively computes and stores the values) over some time interval:  

```{r solve_ross_dts_1, echo=TRUE}
# INPUTS
# pars - the parameters, as a list 
# x0   - the initial value of x
# y0   - the initial value of y
# t0   - the initial value of t
# tmax - the xy value of t 
#
# OUTPUTS
# the values of the variables over time, as a list 
solve_ross_dts_1 = function(pars, x0=.01, y0 = 0.001, t0=0, tmax=100){
   xy = c(t=t0, x=x0, y=y0) 
   xy_t = xy 
   for(t in (t0+1):tmax){
   ...
   return(list(time=xy_t[,1], x=xy_t[,2], y = xy_t[,3], xy = xy)) 
}
```
...
r1out <- solve_ross_dts_1(par_ross_dts, tmax=40)
plot_xy(r1out, type="p") 
```
...
```{r, echo=TRUE}
xy[-1] - ross_dts_1(xy, par_ross_dts)[-1]
```
...
```{r, echo=TRUE}
ross_dts_steady_0(par_ross_dts)
```
...
```{r, echo=TRUE}
par_ross_dts_1 = par_ross_dts
par_ross_dts_1$m = 0.005
ross_dts_steady_0(par_ross_dts_1)
```

What happens if we simulate this? If we set the initial values to be reasonably high, we notice that they apprear to be approaching zero asymptotically. This is good. Since the equations are describing changes in prevalence, we don't want them to ever return negative value. 

```{r ross_dts_1_zero, echo=TRUE}
plot_xy(solve_ross_dts_1(par_ross_dts_1, x0 = .5, y0=.3, tmax=1500)) 
```

On the other hand, our function `ross_dts_steady_0` gives us a negative number.
...
```{r, echo=TRUE}
m_crit = with(par_ross_dts, s*u/a^2)
m_crit 
```
...
```{r, echo=TRUE}
par_ross_dts_2 = par_ross_dts
par_ross_dts_2$m = m_crit
ross_dts_steady_0(par_ross_dts_2)
```
...
```{r, echo=TRUE}
ross_dts_steady_1(par_ross_dts_1)
```

In the next chapter, when we introduce the Ross-Macdonald model, we will revisit this concept of a threshold criterion.  

### Verification 

If we're going to take ourselves seriously, we want to get used to double checking all the code we write. We want to avoid inserting mistakes, and we want to assure anyone who uses the code that it is error free. One way to do this is to find two or more ways of computing the same thing, for verification.

If we've done everything right, we ought to get the same values for the steady states through our analysis and simulation. 

The xy values of $x$ and $y$ from the simulation are: 

```{r, echo=TRUE}
xy[-1]
``` 

The xy values of $x$ and $y$ from evaluating `ross_dts_steady_1()` are:  

```{r, echo=TRUE}
ross_dts_steady_1(par_ross_dts) 
```

It's tempting to look at the printout and assume these two numbers are exactly equal, but when we deal with computers, everything gets computed and stored in some way. We can never be sure that a match will be exact. One simple measure of accuracy is to simply sum up the absolute values of the differences: 

```{r, echo=TRUE}
sum(abs(xy[-1] - ross_dts_steady_1(par_ross_dts)))
```
...
# OUTPUTS
# boolean - 
verify_ross_dts_1 = function(xy, par_ross_dts, tol=1e-9){
  sum(abs(xy[-1] - ross_dts_steady_1(par_ross_dts)))  < tol
}
```

```{r, echo=TRUE}
verify_ross_dts_1(xy, par_ross_dts)
```
...
par_ross_dts_3 = par_ross_dts
par_ross_dts_3$m = 20 
plot_xy(solve_ross_dts_1(par_ross_dts_3, x0 = .01, y0=.001, tmax=20), "b") 
```
...
# OUTPUTS
# the values of the variables over time, as a list 
solve_ross_dts_2 = function(pars, x0=.01, y0 = 0.001, t0=0, tmax=100){
   xy = c(t=t0, x=x0, y=y0) 
   xy = c(t=0, x=x0, y=y0) 
   xy_t = xy 
   for(t in (t0+1):tmax){
...
plot_xy(solve_ross_dts_2(par_ross_dts_3, x0 = .01, y0=.001, tmax=20), "b") 
```
...
```{r, echo=TRUE}
ross_dts_steady_2(par_ross_dts_3)
```

Second, by getting the solution after many iterations. 

```{r, echo=TRUE}
solve_ross_dts_2(par_ross_dts_3, x0=.1, y0=.05,tmax=300)$xy[-1]
```
...
```{r, echo=TRUE}
verify_ross_dts_2 = function(pars, x0 = .1, y0=.05, tmax=500, tol=1e-7){
  xyt = solve_ross_dts_2(pars, x0=x0, y0=y0, tmax=tmax)
  sum(abs(xyt$xy[-1] - ross_dts_steady_2(pars))) < tol
}
```
...
```{r, echo=TRUE}
verify_ross_dts_2(par_ross_dts_3)
```
...
If individuals in a population followed the simple rules described in Equation \@ref(eq:Mprocess) and Equation \@ref(eq:Hprocess), what would they look like? We would like to be able to understand how something we can observe would be connected to the deterministic equations we developed in Section \@ref(dtsRoss). Setting aside the problems that get introduced by realism, sampling, and diagnostics for the moment (we will pretend we're omniscient and that we live in the simple universe we've described) we would find changes occurring at random:

+ individual humans would be switching states, from infected to uninfected and back, at random; 
...
The world really has a random component to it. It is not deterministic but stochastic. In a real study, we can't pretend to be omniscient, but we will discuss that later. 
...
In models with stochasticity, we could use the same exact rules to specify what is expected to happen, but we must draw a random number to determine what happens. To compute the values of these variables, we want to be working with the counting numbers -- the number of humans in a population and among those, the number infected; and the number of mosquitoes in a population, and among those, the number infected. The variability in the values of our variables over time due to the random component of parasite infection dynamics -- herein called parasite demographics -- would be called **demographic stochasticity.**  The model has *some* things in common with the ones we described above, but there are some important differences. The model doesn't have a steady state, but there is something like a steady state that the variables fluctuate around. We'll call it the attractor. The variables are always pulled towards the attractor, but since the process is random, the values of the variables bounce around it.  

The parts of the model are the same. 

**Parameters** -- We can use the same parameters as we did for the `ross_dts` models, but there are two new parameters:

+ $M$ is the number of mosquitoes: $M$ is a counting number 

+ $H$ is the number of humans; $H$ is a counting number

Otherwise we use the same parameters as we did before. We use the stemname `ross_sdts` to denote a Ross-style stochastic discrete-time system. 
...
par_ross_sdts$m = with(par_ross_sdts, M/H)
```
...
Now we have got to describe a stochastic process. Each individual changes state with some probability. Each one is a Bernoulli random variable (*i.e.* either infected or not), so the appropriate probability mass function is the binomial model. In R, we use the built-in function `rbinom(1, N, p)` to draw one random variable on $N$ individuals that remains in the same state with probability $p$. 

In each step, there are four possible changes, depending on the states:

+ $X$ - if humans are infected today, they might lose infection. An infection is lost with probability $s$, so the fraction that remain infected is $1-s.$ The number that remains infected is computed as `rbinom(1, X, 1-s).` 

+ $H-X$ - if humans are uninfected today, they might get infected. The probability of getting infected is called the *daily attack rate.*  The expected number of infected bites is $aY/H$, and if we use the Poisson to model the distribution of bites per person, then the attack rate is $1-e^{-aY/H}$. The number that remains ends up infected is computed as `rbinom(1, H-X, ar).` 

+ $Y$ - if mosquitoes are infected today, they might die. The fraction dying is $u$, so the number of surviving infected mosquitoes is computed as `rbinom(1, X, 1-s).` 

+ $H-Y$ - if mosquitoes are uninfected today, they might get infected. To get infected, a mosquito must blood feed on an infected human, so the fraction getting infected is $aX/H$. The number that gets infected is computed as `rbinom(1, H-Y, a X/H).` 
...
sim_ross_sdts = function(pars, X0=2, Y0 = 1, t0=0, tmax=100){
   ...
   x=X/pars$H 
   y=Y/pars$M 
   return(list(time=XY_t[,1], x=x, y=y, X=X, Y=Y, xy = XY)) 
}
```
...
A useful feature of R is the ability to replicate any particular sequence of random numbers by setting a *seed.* This also makes it easy to write a book knowing what the output will look like. 

```{r, echo=TRUE}
set.seed(253)
XYt <- sim_ross_sdts(par_ross_sdts, tmax=300) 
```
...
The system tends towards a point -- we'll call it an attractor and not a steady state. The values of $X$ and $Y$  (or of $x$ and $y$) fluctuate around that steady state. Since the counting number values get repeated, we can illustrate what the cloud looks like by jittering the locations of the points:
...
points(mX, mY, col = "darkred", pch = 7, cex =2)
```
...
```{r}
c(mx, my) - ross_dts_steady_2(par_ross_sdts)
```
...
solve_ross_xde = function(pars, x0=.01, y0=0.001, tmax=100, dt=1){
...
```{r}
ode_out <- solve_ross_xde(ross_xde_par)
```
...
```{r}
ode_out1 <- solve_ross_xde(ross_xde_par, x0=0.2, y0=0.02)
ode_out2 <- solve_ross_xde(ross_xde_par, x0=.99, y0=0.99)
```
...
```{r, echo=F}
solve_ross_xde(ross_xde_par_5, x0=0.01, y=0.02, tmax=200, dt=1) -> r_xde
...
```{r, echo=F}
solve_ross_dts_2(par_ross_dts_5, x0=0.01, y0=0.02, tmax=200) -> r_dts
solve_ross_dts_1(par_ross_dts_5, x0=0.01, y0=0.02, tmax=200) -> r1_dts
sim_ross_sdts(par_ross_dts_5, X0=1, Y0=1, tmax=200) -> s_dts0
sim_ross_sdts(par_ross_dts_5, X0=1, Y0=1, tmax=200) -> s_dts1
sim_ross_sdts(par_ross_dts_5, X0=1, Y0=1, tmax=200) -> s_dts2
sim_ross_sdts(par_ross_dts_5, X0=1, Y0=1, tmax=200) -> s_dts3
sim_ross_sdts(par_ross_dts_5, X0=1, Y0=1, tmax=200) -> s_dts4
sim_ross_sdts(par_ross_dts_5, X0=1, Y0=1, tmax=200) -> s_dts5
```
