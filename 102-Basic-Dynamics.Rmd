...
We start out describing Ross's quantitative logic before presenting the mathematical models. 

1. Section \@ref(dtsRoss) presents a basic deterministic discrete-time system. Since it is the first model in the book, we walk through it step-by-step, discussing the parts and going slowly.

2. In Section \@ref(basicComputation), we introduce some basic issues about coding and computation.  

3. In Section \@ref(countingEvents), we show that the model presented in Section \@ref(dtsRoss) is numerically unstable. To *fix* the numerical instability, we introduce the Poisson distribution. We use it to develop a new model that is numerically stable. 
...
6. Section \@ref(sxdeRoss) presents a stochastic process in continuous-time; 

7. In Section \@ref(mathSpecies), we compare and contrast discrete *vs.* continuous, and deterministic *vs.* stochastic models. 

8. In the last section, we discuss some of the history of using discrete-time systems for infectious diseases. 
