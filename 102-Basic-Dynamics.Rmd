...
Ross wanted to understand why, so he developed mathematical models of malaria in order to understand the factors affecting the prevalence of malaria infection among humans in a population.
...
To understand this diagram as causal, we must thus imagine that it describes a dynamical process in populations: infected mosquitoes today are biting and infecting humans tomorrow; and infectious humans today are causing infections in mosquitoes tomorrow when they take a blood meal. 
...
We imagine populations that are here comprised of individual mosquitoes and individual humans, and we are interested in computing the fraction of humans and mosquitoes that is infected. 
...
In mathematical formulations, each dying infected mosquito gets replaced by an uninfected mosquito through *emergence* from aquatic habitats. 
If not, the mosquito population would declining over time.
...
One variable in this model is time ($t$). 
Time is marching forward, so we call it the *independent variable.* 
The other variables are computed with respect to time, so we call them *dependent variables.*
The values of dependent variables change over time; they are the main quantities that we want to compute, and the equations describe how the dependent variables changes over time. 

In this model, there are two dependent variables: the proportion of humans and mosquitoes that are infected at each point in time. Since it is a discrete time system, the values of the variables are defined only at integer values of $t.$ 
...
In the code that we wrote, we have adopted some conventions so that the R code closely resembles the equations. In R, the object `xy` is a *named vector.* The names are invisible when we compute anything with `xy,` but the names attach if we use the function `as.list()` or `data.frame().` When combined with the `with()` function, we can create a context where we can call them by name: 
...
+ $- s x_t$ -- describes clearance of human infections: the fraction of humans who are infected is $x_t$; a fraction $s$ clear infections.
    
+ $+ may_t (1-x_t)$ -- describes infection of uninfected humans: the fraction of humans who are infected is $1-x_t$; a fraction $m a y_t$ gets infected. (If $may_t >1,$ the term does not make sense. In sub-section \@ref(dtsRoss2), below, we discuss this in greater depth.)

+ $-u y_t$ -- describes mosquito mortality: the fraction of mosquitoes who are infected is $y_t$; a fraction $u$ die. 
    
+ $+ a x_t (1-y_t)$ -- describes mosquito infections: the fraction of mosquitoes who are not infected is $1-y_t$; a fraction $a x_t$ blood feeds on an infected human and gets infected. 
...
We iterate in two steps: first, we compute the values, and second, we store the values:
...
Computers are very good at repeating tasks, but if we want to get the computers to repeat tasks for us, we must write the code to do it. One useful trick is to write functions that wrap new functionality around other functions to expedite doing work. In this case, we want to write a function that *solves* the equations (*i.e.* that iteratively computes and stores the values) over some time interval:  
...
ross_dts_solve_1 = function(pars, x0=.01, y0 = 0.001, tmax=100){
   xy = c(t=0, x=x0, y=y0) 
   xy_t = xy 
   for(t in 1:tmax){
...
```{r plot_xy}
# INPUTS
# xy_t - a list with named vectors t, x, and y
# add_points - if TRUE, then type = "b" for plot 
#
# OUTPUTS
# the values of the variables over time, as a list 
...
```{r}
plot_xy(ross_dts_solve_1(ross_dts_par, tmax=40), add_points=TRUE) 
```
...
The most obvious solution to these equations is $x=y=0,$ when there is no malaria. We call it the disease-free steady state. The equilibrium makes some sense: if we start with no infected mosquitoes or infected humans in this deterministic model, there can never be any. 
...
```{r ross_dts_steady_0}
# INPUTS
# par - the parameters, as a list 
#
# OUTPUTS
# the steady state values of x and y 
ross_dts_steady_0 = function(par){with(par,{
...
# OUTPUTS
# the steady state values of x and y 
ross_dts_steady_1 = function(par){with(par,{
...
ross_dts_par3$m = 20 
plot_xy(ross_dts_solve_1(ross_dts_par3, x0 = .01, y0=.001, tmax=20)) 
```
...
# OUTPUTS
# the updated values of the variables, as a named vector
ross_dts_2 = function(xy, p){with(as.list(xy), with(p,{
...
# OUTPUTS
# the values of the variables over time, as a list 
ross_dts_solve_2 = function(pars, x0=.01, y0 = 0.001, tmax=100){
   xy = c(t=0, x=x0, y=y0) 
   xy_t = xy 
   for(t in 1:tmax){
...
```{r}
plot_xy(ross_dts_solve_2(ross_dts_par3, x0 = .01, y0=.001, tmax=20)) 
```
...
```{r}
ross_dts_checkit_2 = function(pars, x0 = .1, y0=.05, tmax=500, tol=1e-5){
...
```{r}
ross_dts_checkit_2(ross_dts_par3, tol = 1e-5)
ross_dts_checkit_2(ross_dts_par3, tol = 1e-6)
```

It's a little unsatisfying to get numerical errors this large, but this is probably as close as we can get without doing a lot of work that would, in the end, give us answers that might be closer by a margin that is not expected to make a difference for our analysis. The operating principle here is that we need to be sure that our code is doing what we think it should, and that it is giving answers that are close enough.
...
```{r}
sim_ross_sdts = function(pars, X0=2, Y0 = 1, tmax=100){
   XY = c(t=0, X=X0, Y=Y0) 
   XY_t = XY 
   for(t in 1:tmax){
   ...
   x=X/pars$H 
   y=Y/pars$M 
   return(list(t=XY_t[,1], x=x, y=y, X=X, Y=Y, last = XY)) 
}
```
...
plot_XY = function(xy_t, add_points = FALSE){with(xy_t,{
  llty = "l"
  if(add_points == TRUE) llty = "b"
  plot(t, X, type = llty, ylim = range(0, X, Y), col = "darkgreen", ylab = "X, Y", xlab = "Time", pch=15)
  lines(t, Y, type = llty, col = "darkorange", pch =19)
})}
```
...
par(mfrow=c(1,2))
xdist= with(XYt, x[-c(1:100)]); mx = mean(xdist)
ydist= with(XYt, y[-c(1:100)]); my = mean(ydist) 
...
with(deout1, {
  lines(time, x, type = "l", col = "darkgreen", lty=2) 
  lines(time, y, col = "darkorange", lty=2)
})
with(deout2, {
  lines(time, x, type = "l", col = "darkgreen", lty=3) 
  lines(time, y, col = "darkorange", lty=3)
})
...
sim_ross_sxde = function(pars, X0=2, Y0=1, tmax=300){
   ...
   return(list(t=XY_t[,1], X=XY_t[,2], Y=XY_t[,3], x = XY_t[,4], y = XY_t[,5])) 
}
```
...
clrs = turbo(4)
par(mfrow = c(2,1))
with(r_xde, plot(time, x, type = "l", ylim = c(0,1), col = clrs[1], ylab = "x, y", xlab = "Time", main = "Humans"))
with(r_dts, lines(t, x, col = clrs[2], lwd=2, lty=3, cex=0.5, pch=15))
with(s_dts0, lines(t, x,  col = clrs[3], lty=2, cex=0.5, pch=3))
with(s_xde0, lines(t, x,  col = clrs[4], lty=2, cex=0.5))

with(r_xde, plot(time,  y, type = "l", ylim = c(0,1), col = clrs[1], ylab = "x, y", xlab = "Time", main = "Mosquitoes"))
with(r_dts, lines(t,  y, col = clrs[2], lwd=2, lty=3, cex=0.5, pch=15))
with(s_dts0, lines(t,  y,  col = clrs[3], lty=2, cex=0.5, pch=3))
with(s_xde0, lines(t,  y,  col = clrs[4], lty=2, cex=0.5))
...
par(mfrow = c(3,1))
with(r_xde, plot(time, x, type = "l", ylim = c(0,1), col = clrs[1], ylab = "x, y", xlab = "Time", main = "Humans"))
with(r_dts, lines(t, x, col = clrs[2], lwd=2, cex=0.5, pch=15))
with(r1_dts, lines(t, x, col = clrs[4], lwd=2, lty=3, cex=0.5, pch=15))

with(r_xde, plot(time,  y, type = "l", ylim = c(0,1), col = clrs[1], ylab = "x, y", xlab = "Time", main = "Mosquitoes"))
with(r_dts, lines(t,  y, col = clrs[2], lwd=2, cex=0.5, pch=15))
with(r1_dts, lines(t,  y, col = clrs[4], lwd=2, lty=3, cex=0.5, pch=15))
...
with(r_xde, plot(time, x, type = "n", ylim = c(0,1), col = clrs[1], ylab = "x, y", xlab = "Time", main = "Humans"))
with(s_dts0, lines(t, x,  col = clrsA[1], cex=0.5))
with(s_xde0, lines(t, x,  col = clrsB[1], cex=0.5))

with(s_dts1, lines(t, x,  col = clrsA[2], cex=0.5))
with(s_xde1, lines(t, x,  col = clrsB[2], cex=0.5))

with(s_dts2, lines(t, x,  col = clrsA[3], cex=0.5))
with(s_xde2, lines(t, x,  col = clrsB[3], cex=0.5))

with(s_dts3, lines(t, x,  col = clrsA[4], cex=0.5))
with(s_xde3, lines(t, x,  col = clrsB[4], cex=0.5))

with(s_dts4, lines(t, x,  col = clrsA[5], cex=0.5))
with(s_xde4, lines(t, x,  col = clrsB[5], cex=0.5))

with(s_dts5, lines(t, x,  col = clrsA[6], cex=0.5))
with(s_xde5, lines(t, x,  col = clrsB[6], cex=0.5))

with(r_xde, plot(time,  y, type = "l", ylim = c(0,1), col = clrs[1], ylab = "y, y", xlab = "Time", main = "Mosquitoes"))

with(s_dts0, lines(t, y,  col = clrsA[1], cex=0.5))
with(s_xde0, lines(t, y,  col = clrsB[1], cex=0.5))

with(s_dts1, lines(t, y,  col = clrsA[2], cex=0.5))
with(s_xde1, lines(t, y,  col = clrsB[2], cex=0.5))

with(s_dts2, lines(t, y,  col = clrsA[3], cex=0.5))
with(s_xde2, lines(t, y,  col = clrsB[3], cex=0.5))

with(s_dts3, lines(t, y,  col = clrsA[4], cex=0.5))
with(s_xde3, lines(t, y,  col = clrsB[4], cex=0.5))

with(s_dts4, lines(t, y,  col = clrsA[5], cex=0.5))
with(s_xde4, lines(t, y,  col = clrsB[5], cex=0.5))

with(s_dts5, lines(t, y,  col = clrsA[6], cex=0.5))
with(s_xde5, lines(t, y,  col = clrsB[6], cex=0.5))
