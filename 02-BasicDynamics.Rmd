...
We can translate Ross's basic description of a process into a model, but first, we need to start using mathematical symbols and notation to represent the process. The model itself is formulated as a set of coupled difference equations, but we will write down the equations last. To get there, we must learn about the parts of a model. 
...
+ **Variables** --- These are the quantities we want to compute. In this model, the variables we compute are the fraction of humans and mosquitoes that are infected at each point in time.  
...
+ **Initial Conditions** --- Since the values of our variables in the next time step (at time $t+1$) depend on their values now (at time $t$), we can't really compute anything unless we can say how the process gets started. What are the values of our variables at the beginning (at time $t=0$)? 
...
+ **Parameters** --- The parameters describe the processes that are occurring in the population, the fraction of humans that clear an infection each day, the fraction of infected mosquitoes that survive. Unlike variables, parameters are passed to the model. 

  + Let $s$ denote the fraction of people who remain infected after one day; $0 < s < 1.$ 

  + Let $p$ denote the fraction of mosquitoes who remain alive after one day; $0 < p < 1.$ 

  + Let $a$ denote the fraction of mosquitoes who blood feed on a human; $0 < a < 1.$ 
...
We write a function that computes and returns the updated values of the variables: 

```{r}
dxy = function(xy, p){with(as.list(xy), with(p,{
  xn = x + m*a*y*(1-x) - s*x
  yn = y + a*x*(1-y) - u*y
  return(c(x=xn, y=yn))
}))}
...
```{r}
# The last value is still stored as xy
xy - dxy(xy, par)
```
...
It's tempting to look at the printout and assume these two numbers are exactly equal. This is the land of computation, so things won't be exact. We can simply sum up the differences: 

```{r}
sum(abs(xy - find_eq(par))) 
```
...
```{r}
sum(abs(xy - find_eq(par))) < 1e-7
```

### Packaging

In policy, if we have a task that we want to repeat, we can simply write a function to do it. In this case, we want to write a function that *solves* the equations (*i.e.* that iteratively computes and stores the values) over some time interval:  

```{r}
solve_dxy = function(pars, x0=.01, y0 = 0.001, tmax=100){
   xy = c(x=x0, y=y0) 
   xy_t = xy 
   for(t in 1:tmax){
     xy = dxy(xy, pars)
   ...
   return(list(t=0:tmax, x=xy_t[,1], y = xy_t[,2])) 
}
```
...
Now, we can just plot solutions: 

```{r}
plot_xy(solve_dxy(par, tmax=30), add_points=TRUE) 
```

### Checking 

All our analysis worked out for the basic models that we chose, but what if we had picked different parameters. There must be some very low level of mosquitoes, for example, where malaria can't reproduce. If we reduce $m$ to $0.005$ and evaluate the expression at the steady state, we get negative values for $x$ and $y$. 
...
```{r}
plot_xy(solve_dxy(par1, x0 = .5, y0=.3, tmax=1500)) 
```
...
par3 = par
par3$m = 20 
plot_xy(solve_dxy(par3, x0 = .01, y0=.001, tmax=20)) 
```
...
If we want to do any analysis with pencil and paper in this new system, we're stuck with the problem of solving this:  

$$(1-e^{-m a^2 x/(u + a x)}) (1-x) = s$$ 

We can, however, numerically solve the equations: 

```{r}
dxy_1 = function(xy, p){with(as.list(xy), with(p,{
  xn = x + (1-exp(-m*a*y))*(1-x) - s*x
  yn = y + a*x*(1-y) - u*y
  return(c(x=xn, y=yn))
}))}
```
...
```{r}
solve_dxy1 = function(pars, x0=.01, y0 = 0.001, tmax=100){
   xy = c(x=x0, y=y0) 
   xy_t = xy 
   for(t in 1:tmax){
     xy = dxy_1(xy, pars)
...
Now, we have fixed our stability problem. 

```{r}
plot_xy(solve_dxy1(par3, x0 = .01, y0=.001, tmax=30)) 
```
